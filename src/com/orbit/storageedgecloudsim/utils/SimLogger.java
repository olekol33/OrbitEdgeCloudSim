...
package com.orbit.storageedgecloudsim.utils;

import com.orbit.storageedgecloudsim.core.SimManager;
import com.orbit.storageedgecloudsim.core.SimSettings;
import com.orbit.storageedgecloudsim.core.SimSettings.NETWORK_DELAY_TYPES;
import com.orbit.storageedgecloudsim.edge_server.EdgeHost;
import com.orbit.storageedgecloudsim.storage.ObjectGenerator;
import com.orbit.storageedgecloudsim.storage.RedisListHandler;
import com.orbit.storageedgecloudsim.utils.SimLogger.NETWORK_ERRORS;
...
//		int o=0;
		...
		for (Map.Entry<Integer, LogItem> entry : taskMap.entrySet()) {
			...
			if (value.getStatus() == TASK_STATUS.COMPLETED) {
				...
				objectStatusID[key-1] = TASK_STATUS.COMPLETED;
			...
			else if(value.getStatus() == TASK_STATUS.CREATED ||
					value.getStatus() == TASK_STATUS.UPLOADING ||
					value.getStatus() == TASK_STATUS.PROCESSING ||
					value.getStatus() == TASK_STATUS.DOWNLOADING)
			{
				ArrayList<Double> delays = new ArrayList<Double>(numOfObjectsInStripe);
				if (timeToReadStripe.get(value.getIoTaskID()) != null)
					delays = timeToReadStripe.get(value.getIoTaskID());
...
//				nameToReadStripe.put(value.getIoTaskID(),value.getStripeID());
			...
			if (value.getStatus() == TASK_STATUS.COMPLETED) {
			...
			} else if (value.getStatus() == TASK_STATUS.REJECTED_DUE_TO_VM_CAPACITY) {
			...
			} else if (value.getStatus() == TASK_STATUS.REJECTED_DUE_TO_BANDWIDTH
					|| value.getStatus() == TASK_STATUS.UNFINISHED_DUE_TO_BANDWIDTH) {
				...
				if (fileLogEnabled && SimSettings.getInstance().getDeepFileLoggingEnabled())
					appendToFile(failBW, value.toString(key));
			} else if (value.getStatus() == TASK_STATUS.UNFINISHED_DUE_TO_MOBILITY) {
